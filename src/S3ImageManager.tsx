...
import { useProducts } from "./ProductsContext"; // Ensure the correct import path
...
const S3ImageManager = () => {
  ...
  const { getProductsUsingImage } = useProducts(); // Adjusted hook to retrieve products using image
  ...
  const confirmDeleteImage = async () => {
    if (imageToDelete && !getProductsUsingImage(imageToDelete.url).length) {
            ...
            >
              ...
              <div className="p-4 flex justify-between items-center">
                <button
                  ...
                  disabled={usingProducts.length > 0}
                >
                  Delete
                </button>
                {usingProducts.length > 0 && (
